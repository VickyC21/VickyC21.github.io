...
大模型应用不是一段孤立的 Python 代码。一次请求会经过[域名解析](/_posts/2026-08-17-domain-name-resolution-dns.md)、网络路由、端口监听、反向代理和 Python 进程；进程还要读取配置与密钥，并以某个操作系统用户身份访问文件、数据库和网络。理解这些层，才能把“服务不可用”变成可验证的问题，而不是反复重启和猜测。
